...
@endsection
@section('judul','Edit Pertanyaan Quiz')
        ...
        <div class="col-md-12  mt-5">
           <div class="card">
              <div class="card-header bg-primary">
                 <h6 class="text-white">Edit Pertanyaan Quiz<div class="float-right kembalimateri" style="cursor :pointer;">X</div> </h6>
              </div>
              <div class="card-body">
                 <form method="post" action="{{url('quizgame/'.$s->id)}}" enctype="multipart/form-data" id="submitdata">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <div class="text-center">
                                    <img class="img" id="loadfotoadd" src="{{$s->gambar}}" alt="Foto Thumbnail"
                                        style=" height:30%; width:30%;">                            
                        </div>

                    <div class="form-group">
                       <label>Soal :</label>
                       <textarea class="summernote_dessription" name="soal" id="isi_materi">{{$s->soal}}</textarea>
                    </div>

                    @foreach($s->gandaquiz as $n=>$g)
                    @if($n == 0)
                    <div class="form-group">
                       <label class="badge badge-success text-white py-2 w-100"
                       style="font-size: 15px;" 
                       >Jawaban (Benar):</label>
                       <textarea class="summernote_dess" required name="jawbenar" id="isi_jawab">{{$g->jawaban}}</textarea>
                    </div>
                    ...
                    <div class="list-jawaban">
                       ...
                       style="font-size: 15px;" 
                       >Jawaban Lain:</label>
                    @foreach($s->gandaquiz as $nq=>$gq)
                    @if($nq > 0)
                    <div class="mb-3">
                    <textarea class="summernote_jaw mb-3" required name="jaw[{{$gq->id}}]">{{$gq->jawaban}}</textarea>
                    </div>
                    @endif
                    @endforeach
                    </div>
